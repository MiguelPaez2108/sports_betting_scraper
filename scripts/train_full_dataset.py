...
Train Logistic Regression and XGBoost with all 18K historical matches.
"""
...
def train_full_dataset():
    """Train models with full 18K dataset"""
    
    console.print(Panel.fit(
        "[bold cyan]Training ML Models - FULL DATASET[/bold cyan]\n"
        "[yellow]18,011 matches (2015-2024)[/yellow]",
        border_style="cyan"
    ))
    
    # Load full data
    console.print("\n[bold yellow]Step 1: Loading Full Dataset[/bold yellow]")
    ...
    with MLDataLoader(DB_CONFIG) as loader:
        data = loader.prepare_training_data(
            ...
            end_date='2024-12-31',
            leagues=['E0', 'SP1', 'I1', 'D1', 'F1'],  # All 5 leagues
            test_size=0.15,
            val_size=0.15,
            batch_size=100,
            use_cache=True,
    ...
    console.print("\n[bold yellow]Step 3: Training XGBoost (with early stopping)[/bold yellow]")
    ...
    comparison_table = Table(title="Model Performance on Test Set (18K Dataset)")
    ...
    lr_model.save(str(models_dir / "logistic_regression_full.pkl"))
    xgb_model.save(str(models_dir / "xgboost_full.json"))
    ...
    console.print(f"[dim]Ready for production predictions[/dim]\n")
